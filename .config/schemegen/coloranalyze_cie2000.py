...
def perceptual_dist(p1, p2):
    p1 = sRGBColor(p1[0], p1[1], p1[2], is_upscaled=True)
    p2 = sRGBColor(p2[0], p2[1], p2[2], is_upscaled=True)
    p1 = convert_color(p1, LabColor)
    p2 = convert_color(p2, LabColor)
...
def get_xcolors(colors, substitution_distance=20):
	...
	for crgb in canon_od.values():
		distances = []
		for rgb in colors:
			distance = perceptual_dist(crgb, rgb)
